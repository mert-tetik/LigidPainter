...
void main()
{
    ...
    for(int i = 0; i < 4; i++){
        worley = getWorleyNoise(rotate(uv, vec3(0,1,0), 35. * float(i)));
        baseColor = mix(baseColor, aMossColorBack, worley);
    }
    
    worley = getWorleyNoise(rotate(uv * frontLayerScale, vec3(0,1,0), 95.));
    float worley2 = getWorleyNoise(rotate(uv / 2. + 0.1, vec3(0,1,0), 95.));
    ...
    float droplets = getDroplets(vec3((uv / (5.*2.)).x * 4., (uv/(5.*2.)).y, (uv/(5.*2.)).z * 4.)); 
