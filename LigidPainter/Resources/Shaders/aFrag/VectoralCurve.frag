...
{	
    ...
    vec2 A = startPos/100.;
    vec2 C = destPos/100.;
    vec2 B = direction/100.;
    
    float maxX = max(max(A.x, B.x), C.x) + 0.02;
    float minX = min(min(A.x, B.x), C.x) - 0.02;
    float maxY = max(max(A.y, B.y), C.y) + 0.02;
    float minY = min(min(A.y, B.y), C.y) - 0.02;
