...
private:
    ...
    std::map<std::string,std::string> uniformData{
        {"tex_coords","vec2"}, {"normal","vec3"}, {"posScene","vec4"} , {"posModel","vec3"} , {"viewPos","vec3"} , {"skyboxExposure","float"},{"blurySkybox","samplerCube"} , {"prefilterMap","samplerCube"}, {"tangent", "vec3"} , {"bitangent", "vec3"},{"brdfLUT","sampler2D"}
    };
    ...
    void createTheCode(std::string code){
        ...
        for (size_t i = 0; i < uniforms.size(); i++)
        {
            ...
            if(uniforms[i] == "viewPos" || uniforms[i] == "blurySkybox" || uniforms[i] == "prefilterMap" || uniforms[i] == "brdfLUT" || uniforms[i] == "skyboxExposure" ){
