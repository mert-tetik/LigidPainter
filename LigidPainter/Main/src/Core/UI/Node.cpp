...
float maxScreenWidth,float maxScreenHeight, std::vector<Node> &nodes,float zoomVal){
	ColorData colorData;
	Utilities util;
	...
	if(node.barPressed){
		node.positionX += xOffset/maxScreenWidth*2.f/zoomVal;
		node.positionY -= yOffset/maxScreenHeight*2.f/zoomVal;
	...
	box(node.width,node.height,node.positionX * zoomVal,node.positionY * zoomVal,"",node.backColor,0,0,0,0.9999f,10000,node.backColor,0);
    //-Side area
	box(iconWidth,node.height,node.positionX * zoomVal-node.width -iconWidth,node.positionY * zoomVal,"",node.backColor,0,0,0,0.9999f,10000,node.backColor,0);///Left
	box(iconWidth,node.height,node.positionX * zoomVal+node.width +iconWidth,node.positionY * zoomVal,"",node.backColor,0,0,0,0.9999f,10000,node.backColor,0);///Right
	box(node.width,iconWidth*2.f,node.positionX * zoomVal,node.positionY * zoomVal + node.height + iconWidth*2.f,"",node.upBarColor,0,0,0,0.9999f,10000,node.upBarColor,0);///Top
	box(node.width,iconWidth*2.f,node.positionX * zoomVal,node.positionY * zoomVal - node.height - iconWidth*2.f,"",node.backColor,0,0,0,0.9999f,10000,node.backColor,0);///Bottom
	//-Smooth corners
	glUseProgram(programs.iconsProgram);
	iconBox(iconWidth , iconWidth*2.f , node.positionX * zoomVal-node.width -iconWidth, node.positionY * zoomVal + node.height + iconWidth*2.f, 0.9999f , icons.TL , 0 , node.upBarColor , node.backColor);
	iconBox(iconWidth , iconWidth*2.f , node.positionX * zoomVal+node.width +iconWidth, node.positionY * zoomVal + node.height + iconWidth*2.f, 0.9999f , icons.TR , 0 , node.upBarColor , node.backColor);
	iconBox(iconWidth , iconWidth*2.f , node.positionX * zoomVal-node.width -iconWidth, node.positionY * zoomVal - node.height - iconWidth*2.f, 0.9999f , icons.BL , 0 , node.backColor , node.backColor);
	iconBox(iconWidth , iconWidth*2.f , node.positionX * zoomVal+node.width +iconWidth, node.positionY * zoomVal - node.height - iconWidth*2.f, 0.9999f , icons.BR , 0 , node.backColor , node.backColor);
	...
	for (size_t i = 0; i < node.outputs.size(); i++)
	{
		...
		if(glfwGetMouseButton(window,0) == GLFW_PRESS && node.outputs[i].connectionHover && !node.outputs[i].pressed){
			...
			node.outputs[i].connectionPosX = node.positionX * zoomVal+node.width +iconWidth*2.f;
			node.outputs[i].connectionPosY = (node.positionY * zoomVal + node.height) - i/(20.f/(node.width*15)) - 0.05f * node.width*10;
			firstRelease = true;
		}
        ...
        glUseProgram(programs.uiProgram);
		drawLine(node.positionX * zoomVal+node.width +iconWidth*2.f,(node.positionY * zoomVal + node.height) - i/(20.f/(node.width*15)) - 0.05f * node.width*10,0.99999f,node.outputs[i].connectionPosX,node.outputs[i].connectionPosY,0,nodeColor);

        //Render the output
		//TODO : Use those values for rendering and tracking
        glUseProgram(programs.iconsProgram);
		node.outputs[i].posX = node.positionX * zoomVal+node.width +iconWidth*2.f;
		node.outputs[i].posY = (node.positionY * zoomVal + node.height) - i/(20.f/(node.width*15)) - 0.05f * node.width*10;
		iconBox(iconWidth/1.5f , iconWidth*1.5f , node.positionX * zoomVal+node.width +iconWidth*2.f, (node.positionY * zoomVal + node.height) - i/(20.f/(node.width*15)) - 0.05f * node.width*10, 0.99999f , icons.Circle , 0 , nodeColor , nodeColor);
		ioIndex++;
	}
	...
	for (size_t i = 0; i < node.inputs.size(); i++)
	{
		...
		node.inputs[i].connectionHover = isMouseOnButton(window,iconWidth/1.5f , iconWidth*1.5f,node.positionX * zoomVal-node.width - iconWidth*2.f,(node.positionY * zoomVal + node.height) - (i+ioIndex+rangeBarCountInputs)/(20.f/(node.width*16)) - 0.05f * node.width*10,mouseX,mouseY,false);

		node.inputs[i].posX = node.positionX * zoomVal-node.width - iconWidth*2.f;
		node.inputs[i].posY = (node.positionY * zoomVal + node.height) - (i+ioIndex+rangeBarCountInputs)/(20.f/(node.width*16)) - 0.05f * node.width*10;

		iconBox(iconWidth/1.5f , iconWidth*1.5f , node.positionX * zoomVal-node.width - iconWidth*2.f, (node.positionY * zoomVal + node.height) - (i+ioIndex+rangeBarCountInputs)/(20.f/(node.width*16)) - 0.05f * node.width*10, 0.99999f , icons.Circle , 0 , nodeColor , nodeColor);
	...
	renderText(programs.uiProgram,node.title,node.positionX * zoomVal-node.width -iconWidth,node.positionY * zoomVal + node.height + iconWidth*1.f,node.width/300.f);
	
    //Render the output titles
	for (size_t i = 0; i < node.outputs.size(); i++)
	{
		renderText(programs.uiProgram,node.outputs[i].text,node.positionX * zoomVal+node.width - (node.outputs[i].text.size()/60.f)*node.width*8.f,(node.positionY * zoomVal + node.height) - i/(20.f/(node.width*16)) - 0.05f * node.width*10,node.width/300.f);
	}
	...
	for (size_t i = 0; i < node.inputs.size(); i++)
	{
		...
		renderText(programs.uiProgram,node.inputs[i].text,node.positionX * zoomVal-node.width -iconWidth + 0.015f,(node.positionY * zoomVal + node.height) - (i+ioIndex+inputElementIndex)/(20.f/(node.width*16)) - 0.05f * node.width*10,node.width/300.f);
		inputElementIndex++;
        ...
        for (size_t k = 0; k < rangeBarCount; k++)
		{
			...
			//Range bar
			box(node.width,iconWidth*2.f,node.positionX * zoomVal,(node.positionY * zoomVal + node.height) - (i+ioIndex+inputElementIndex)/(20.f/(node.width*16)) - 0.05f * node.width*10,"",colorData.rangeBarBack,0,0,0,0.99998f,8 / (node.width*6),node.backColor,0);///Bottom
			//Pointer
			box(iconWidth/4.f,iconWidth*2.f,node.positionX * zoomVal + pointPosVal,(node.positionY * zoomVal + node.height) - (i+ioIndex+inputElementIndex)/(20.f/(node.width*16)) - 0.05f * node.width*10,"",colorData.rangeBarFront,0,0,0,0.99999f,8 / (node.width*6),node.backColor,0);///Bottom

			//TODO : Calculate the screen gap
            //Check if range bar pointer pressed
			if(!anyPointerPressed){
				isRangeBarPointerHover = isMouseOnButton(window , iconWidth , iconWidth*2.f ,node.positionX * zoomVal + pointPosVal,(node.positionY * zoomVal + node.height) - (i+ioIndex+inputElementIndex)/(20.f/(node.width*16)) - 0.05f * node.width*10,mouseX,mouseY,false);
			}
