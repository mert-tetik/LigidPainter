...
,Font &txtrGenSelectedFont,unsigned int &generatedTextTxtr,int &TDModelsPanelScrollVal) {
	...
	if(panelData.exportPanelActive || panelData.modelPanelActive || panelData.paintingPanelActive || panelData.texturePanelActive || panelData.settingsPanelActive || panelData.generatorPanelActive){ //Disable panel if a message box is active
		...
		if(addNodeContextMenu.active && addNodeContextMenu.buttons.size() > 1){
			...
			if(addNodeContextMenu.buttons.size() > 9){
				const bool searchButtonEnter = ui.isMouseOnButton(renderData.window,addNodeContextMenu.buttons[1].width, addNodeContextMenu.buttons[1].height, addNodeContextMenu.positionX-screenGapX, addNodeContextMenu.positionY - 0.265f * 2.05,mouseXpos,mouseYpos,false); 
				...
				ui.box(addNodeContextMenu.buttons[1].width, addNodeContextMenu.buttons[1].height, addNodeContextMenu.positionX, addNodeContextMenu.positionY - 0.265f * 2.05, addNodeContextMenu.searchText, selectedTextboxColor, 0.05f,false, false, 0.99f, addNodeContextMenu.buttons[1].buttonCurveReduce, selectedTextboxColor, addNodeContextMenu.searchTextboxActive); //Add mask texture button	
				glUseProgram(programs.iconsProgram);
				if(!addNodeContextMenu.searchTextboxActive)
				...
				if(addNodeContextMenu.scroll != addNodeContextMenu.buttons.size()-10 && addNodeContextMenu.buttons.size() > 9){	
					ui.iconBox(0.01f,0.02f,addNodeContextMenu.positionX, addNodeContextMenu.positionY - 0.265*1.87f, 0.9999,icons.ArrowDown,0,colorData.iconColor,colorData.iconColor);
				}
			}
		}
		else{
	...
	for (size_t i = 0; i < UIElements.size(); i++)
	{
		...
		if(panelCompatibility && !(UIElements[UIdynamicPaintingCheckBoxElement].checkBox.checked && (i == UIbrushBordersRangeBar|| i == UIbrushBordersText || i == UIbrushRotationRangeBar|| i == UIbrushRotationText || i == UIbrushSpacingRangeBar || i == UIbrushSpacingText || i == UIuseNegativeCheckBox || i == UIaddBrushMaskTextureIcon ||  i == UIselectBrushMaskTextureIcon || i == UImaskTextureFileNameText))){
			...
			if(currentType == "button"){
				ui.box(UIElements[i].button.width, UIElements[i].button.height, centerCoords - screenGapX + UIElements[i].button.positionX, UIElements[i].button.positionY+slideVal, UIElements[i].button.text, glm::vec4(colorData.buttonColor.r,colorData.buttonColor.g,colorData.buttonColor.b,1.f), UIElements[i].button.textRatio, false, false, UIElements[i].button.positionZ, UIElements[i].button.buttonCurveReduce, UIElements[i].button.colorHover, UIElements[i].button.transitionMixVal,glm::vec4(colorData.buttonColor.r,colorData.buttonColor.g,colorData.buttonColor.b,1.f),colorData.buttonColorHover,0.00022f,true); //Add mask texture button
			}
			if(currentType == "text"){	
			...
			if(currentType == "textBox"){
				ui.box(UIElements[i].textBox.width, UIElements[i].textBox.height,centerCoords - screenGapX + UIElements[i].textBox.position_x, UIElements[i].textBox.position_y+slideVal,UIElements[i].textBox.text , colorData.textColor, 0 , true, false, UIElements[i].textBox.position_z, 10 , colorData.textBoxColorClicked, UIElements[i].textBox.transitionMixVal,colorData.textColor,colorData.textColor,0.00022f,true); //Add mask texture button
			}
	...
	if(panelData.settingsPanelActive){
		resolutionChanged = ui.listBox(centerCoords - screenGapX,0.53f+panelData.settingsPanelSlideVal,0.9f,"Texture Resolution",0.1f,icons,{"256","512","1024","2048","4096"},true,renderData.window,mouseXpos,mouseYpos,firstClick,chosenTextureResIndex,screenGapX);
		ui.listBox(centerCoords - screenGapX,0.06f+panelData.settingsPanelSlideVal,0.9f,"Node Resolution",0.1f,icons,{"256","512","1024","2048"},true,renderData.window,mouseXpos,mouseYpos,firstClick,chosenNodeResIndex,screenGapX);
