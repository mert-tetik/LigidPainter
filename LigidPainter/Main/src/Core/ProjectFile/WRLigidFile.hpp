        ...
        for (size_t sceneI = 0; sceneI < nodeSceneSize; sceneI++)
        {
            ...
            for (size_t nodeI = 0; nodeI < nodeSize; nodeI++)
            {
                ...
                for (size_t frI = 0; frI < nodeFragSourceSize; frI++)
                {
                    char c;
                    rf.read(reinterpret_cast<char*>(&c),sizeof(char));
                    nodeScenes[sceneI].nodes[nodeI].fragSource.push_back(c);
                }
		            ...
		            const char* shaderSource = nodeScenes[sceneI].nodes[nodeI].fragSource.c_str();
		            ...
		            nodeScenes[sceneI].nodes[nodeI].program = glCreateProgram();
		            glAttachShader(nodeScenes[sceneI].nodes[nodeI].program, vertShader);
		            glAttachShader(nodeScenes[sceneI].nodes[nodeI].program, fragShader);
		            glLinkProgram(nodeScenes[sceneI].nodes[nodeI].program);
    ...
    void writeNodeScenes(std::ofstream &wf,std::vector<NodeScene> nodeScenes){
        ...
        for (size_t sceneI = 0; sceneI < nodeScenes.size(); sceneI++)
        {
            ...
            for (size_t nodeI = 0; nodeI < nodeScenes[sceneI].nodes.size(); nodeI++)
            {
                ...
                uint64_t nodeFragSourceSize = nodeScenes[sceneI].nodes[nodeI].fragSource.size();
                wf.write(reinterpret_cast<char*>(&nodeFragSourceSize),sizeof(uint64_t));
                for (size_t frI = 0; frI < nodeFragSourceSize; frI++)
                {
                    wf.write(reinterpret_cast<char*>(&nodeScenes[sceneI].nodes[nodeI].fragSource[frI]),sizeof(char));
                }
