...
void checkComboList_painting_over_render(Timer& timer, bool doMouseTracking){    
    ...
    if(checkComboList_painting_over.panel.sections[0].elements[2].button.clicked){
        ...
        if(texture.ID){
            ...
            registerTextureFieldAction("New texture field via texture selection dialog", getTextureFieldScene()->texture_fields);
        ...
        if(test.size()){
            ...
            registerTextureFieldAction("New texture field via path", getTextureFieldScene()->texture_fields);
            getTextureFieldScene()->add_new(TextureField(texture));
        }
    }
}