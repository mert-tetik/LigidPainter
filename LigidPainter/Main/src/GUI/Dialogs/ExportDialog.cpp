...
static void exportLibraryMaterials(Panel& propertiesPanel){
    ...
    for (size_t i = 0; i < Library::getMaterialArraySize(); i++)
    {
        ...
        {
            Library::getMaterialObj(i).materialModifiers[modI].updateMaterialChannels(*Library::getMaterial(i), matMesh, resolution, modI, Settings::appTextures().white, 0, false);
        }
