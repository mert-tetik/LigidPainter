...
bool verify_exit_text_asserted = false;
...
void LogDialog::talking_interaction(Timer& timer){
    ...
    if(getContext()->window.shouldClose()){
        ...
        verify_exit_text_asserted = false;
    }

    if(this->is_cat_talking())
        return;
    ...
    // Return if comments are not allowed
    if(!Settings::properties()->cat_allowComments)
        return;
