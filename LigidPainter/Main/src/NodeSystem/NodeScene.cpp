...
void NodeScene::render(Timer &timer,  Panel nodeEditorPanel, NodePanel& nodePanel, bool doMouseTracking){
...
Mesh NodeScene::processNode(Node &node, Mesh& mesh, int textureRes){
    ...
    if(node.nodeIndex == MATERIAL_ID_NODE){        
        ...
        {
            material.materialModifiers[i].updateMaterialChannels(material, msh, textureRes, i, appTextures.white, 0, false);
        }
